...
class ModListPage {
public:
    virtual ~ModListPage() = default;
    virtual void addModSource(ModSource&& source) = 0;
};
...
public:
    ...
    using PageLoadTask = Task<Result<std::shared_ptr<ModListPage>, LoadPageError>, std::optional<uint8_t>>;
    using PageCreator = std::function<std::shared_ptr<ModListPage>()>;
...
protected:
    std::unordered_map<size_t, std::shared_ptr<ModListPage>> m_cachedPages;
...
public:
    ...
    PageLoadTask loadPage(size_t page, PageCreator&& pageCreator, bool forceUpdate = false);
